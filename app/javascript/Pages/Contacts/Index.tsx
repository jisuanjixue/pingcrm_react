...
import { DataTable, EmptyState, Form, FormLayout, SearchInput, Select, useModals } from "@saas-ui/react";
  ...
  return (
    <>
      ...
      <Flex mb="6" alignItems="center" justifyContent="space-between">
        <Form onSubmit={handleSubmit} onReset={handleReset}>
          <FormLayout columns={[1, null, 6]}>
            ...
            <SearchInput placeholder="Search" value={data.search} onChange={e => handleChange(e.target.value, "search")} onReset={() => handleChange("", "search")} />
      ...
      <HStack mb="2" spacing="900">
        <Button onClick={() => tableRef.current.toggleAllRowsSelected(!tableRef.current.isAllRowsSelected)}>Select all rows</Button>
        <Button leftIcon={<AddIcon />} colorScheme="teal" variant="solid">
          <Link href={Routes.new_contact()}>Create Contact</Link>
        </Button>
      </HStack>
      <Box overflowY="auto" borderRadius="1" bgColor="#ffffff" boxShadow="md">
        {contacts?.data.length > 0 ? (
            ...
            data={contacts?.data.map(v => ({
              ...
              name: (
                <HStack spacing="24px">
                  ...
                  <Box w="40px" h="40px">
                    <IconButton colorScheme="red" variant="outline" icon={<DeleteIcon />} aria-label="Delete" />
                  </Box>
                </HStack>
              ),
              action: (
                <ButtonGroup variant="solid" size="sm" spacing={3}>
                  <IconButton colorScheme="green" icon={<EditIcon />} aria-label="Edit" onClick={() => Inertia.get(Routes.edit_contact(v.id))}></IconButton>
                  <IconButton
                    colorScheme="red"
                    icon={<DeleteIcon />}
                    aria-label="Delete"
          ...
          <EmptyState
            ...
            actions={
              <>
                <Button colorScheme="teal" variant="solid" onClick={() => Inertia.get(Routes.new_contact())}>
                  Create contacts
                </Button>
