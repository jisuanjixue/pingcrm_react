...
const FileUpload = ({ setData, label, photo, errors }: IProps) => {
  ...
  return (
    <>
      ...
      {photo ? <Box pr={8} pl={8}>
        <Button onClick={() => browse()} colorScheme='teal' variant='solid' >Browse</Button>
      </Box> :
        <Box display="flex" alignItems="center" justifyContent="space-between" pr={8} pl={8}>
          <Box>{photo?.name}</Box>
          <Text fontSize="" color="">{filesize(photo?.size)}</Text>
          <Button onClick={() => handRemove()} colorScheme='teal' variant='solid'>Remove</Button>
        </Box>
      }
      {errors.length > 0 && <FormErrorMessage>
        {errors[0]}
      </FormErrorMessage>}
    </>
  )
